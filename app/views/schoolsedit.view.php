      ...
      <?php 
             if (isset($sucees)) {
              ...
              <?php foreach ($stmt as $key ) {
                 // code...
              ?>
      <form method="POST" enctype="multipart/form-data" action="<?php echo(url('schools/edit'))?>">
         <div class="form-outline mb-4">
            <label for="Name" class="form-label"> Name</label>
            <input type="text" name="Name" id="Name" placeholder="Enter  Name of the schoole" class="form-control" autocomplete="off" value="<?php echo $key['school']?>">
         </div>
         ...
         <button type="submit" name="edit" value="<?php echo $key['id']?>" class="btn btn-primary py-2 px-3 Border-0 float-end">submit</button> 
             <a href="<?php echo url('schools')?>" class="text-light btn btn-warning">Cancel</a>  <?php }?>
            </div>
